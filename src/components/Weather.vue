<template>
  <div class="flex flex-col items-center justify-center w-screen text-gray-700 px-6 pt-5 bg-gradient-to-br">
    <div class="w-full max-w-screen-sm bg-white p-4 rounded-xl ring-8 ring-white ring-opacity-40">
      <div class="flex justify-between">
        <div class="flex flex-col">
          ...
          <span class="text-6xl font-bold">
            <span v-if="nowWeather">{{ Math.floor(nowWeather.temperature) }}°C</span>
          </span>
          <div class="flex cols-2 gap-4">
            ...
            <div class="text-xs font-light text-gray-400">
              Осадки:
              <span v-if="nowWeather">{{ nowWeather.precipitation_probability }} %</span>
            </div>
          </div>
        </div>
      ...
      <div class="flex snap-x snap-mandatory overflow-x-auto mt-6 lg:mt-12">
        <div class="snap-start shrink-0 w-1/4 sm:w-1/5 md:w-1/6" v-for="hour in forecast">
          <div class="flex flex-col items-center">
            ...
            <img :src="hour.img" alt="" class="h-8 fill-current text-gray-400 mt-3" />
